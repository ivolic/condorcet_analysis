...
def drop_candidate(vprofile, candidates, method):
    lowest_score = {"candidates": [], "score": None}
    
    if method == "PM":
        scores = mm.Borda_PM_Return_Full(vprofile, candidates, tiebreak="first_place")
    elif method == "OM":
        scores = mm.Borda_OM_Return_Full(vprofile, candidates, tiebreak="first_place")
    elif method == "AVG":
        scores = mm.Borda_AVG_Return_Full(vprofile, candidates, tiebreak="first_place")
...
def main_helper(vprofile, candidates, method, round_num=1):
    logger.info(f"Round {round_num}: Remaining candidates: {candidates}")
    if len(candidates) == 1:
        return candidates[0]
    else:
        cand = drop_candidate(vprofile, candidates, method)
        cand_to_drop = ""
        if len(cand) == 1:
            cand_to_drop = cand[0]
        else:
        ...
        return main_helper(vprofile, c, method, round_num + 1)

        
def run_tvr(full_path):
    logger.info(f"\n{'='*60}")
    ...
    for m in ["OM", "PM", "AVG"]:
        ...
        winner = main_helper(vprofile, candidate_new, m)

...
def diversity_score_simplex(profile, cands, diagnostic=False):
    ...
    while not cond_winner:
        ...
        diversity_scores = {}
        for cand in hopefuls:
            dist = np.linalg.norm(simplex_points[cand] - np.ones(math.factorial(len(hopefuls)-1))/math.factorial(len(hopefuls)-1))
            if dist>0:
                diversity_scores[cand] = 1/dist
            else:
                diversity_scores[cand] = float('inf')
...
###############################################################################
###############################################################################
##### Different TVR models for truncated borda scores
###############################################################################
...
###########################################
##### Friendly Fire Condorcet Method(s)
###########################################

def friendly_fire_inst(profile, cands, diagnostic=False):
    ...
    for cand in smith_set:
        loses_to = [cands[i] for i in range(num_cands) if (margins[cands.index(cand), i]<=0 and cands[i]!=cand)]
        if diagnostic:
            print(cand, loses_to)
        # Borda PM score
        borda_score = 0
...
def friendly_fire_inst_smith_exp(profile, cands, diagnostic=False):
